...
The Robot navigates using a multistep process. First a C-space removes all points inaccessible to the robot from the map. Next unknown points that border known areas of the map are selected and grouped based on adjacency. These groups are filtered by size to remove noise. The centroids of each group are calculated and set as the destination. We use a modified A* algorithm combined with a recursive shadowcasting algorithm to efficiently determine a path to the destination. Once a path is completed the robot will only recompute the closest 5 points on the path as it travels. This allows the robot to update path trajectories in real time. The robot will continuously path towards the closest unexplored area until the map is fully discovered.

![Video Project 5](https://github.com/user-attachments/assets/40a74386-2fed-4ab9-9ccb-dac3e39044db)

<img width="260" height="330" alt="image" src="https://github.com/user-attachments/assets/c9421315-73ca-4b7c-94a0-a828a79ddd94" />
